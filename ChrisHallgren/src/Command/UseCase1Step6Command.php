 ...
 */
class UseCase1Step6Command extends Command
{
    ...
    {
        $io->out('Creating Purchase Order');
        ...
        $query = <<<'QUERY'
mutation UseCase1Step6Command($transactions_create: CreateTransactions_TransactionInput!) {
...
QUERY;
        ...
        if ($response->getErrors()) {
            ...
            $io->hr();
            $io->out('Created Purchase Order ' . $data['header']['referenceNumber'] . ' With ID: ' . $data['id'] . ' Date: ' . $data['header']['txnDate']);
        }
    }
}
